...
/*   Updated: 2021/01/16 19:08:40 by iidzim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct  s_sprite
{
	...
	int         w;
	int         h;
	double      **sprite; // [][0] = x;  [][1] = y; [][2] = dist;//freed
	t_point     spr;
...
{  
	...
	t_point         raydir; //direction of ray
	t_point         plane; //the 2d raycaster version of camera plane
	t_point         camera; //coordinate in camera space
	t_point         sidedist; //length of ray from current position to next x or y-side
	t_point         deltadist; //length of ray from one x or y-side to next x or y-side
	...
	int             drawend;
	int             lineheight;
	double          perpwalldis; //distance projected on camera direction
	t_sprite        s;
	double          *buff;
}                   t_data;
...
//gcc -g  -Wall -Wextra -Werror -L ./minilibx_opengl_20191021 
//-lmlx -framework OpenGL -framework AppKit  *.c -fsanitize=address
